...
class ObjectFindingMixin:
    ...
    def find_max_cell_location(self, field, finest_levels=3):
        ...
        mylog.debug(
            "Searching %s grids for maximum value of %s", len(source._grids), field
        )
    ...
    def find_min(self, field):
        ...
        mylog.info(
            "Min Value is %0.5e at %0.16f %0.16f %0.16f in grid %s at level %s",
            minVal,
            pos[0],
            pos[1],
            pos[2],
    ...
    def find_sphere_grids(self, center, radius):
        ...
        DW = self.dataset.domain_right_edge - self.dataset.domain_left_edge
    ...
    def get_box_grids_below_level(self, left_edge, right_edge, level, min_level=0):
    ...
    def get_periodic_box_grids_below_level(
        self, left_edge, right_edge, level, min_level=0
    ):
        mask = np.zeros(self.grids.shape, dtype="bool")
        dl = self.dataset.domain_left_edge
        dr = self.dataset.domain_right_edge
        ...
        for off_x in [-1, 0, 1]:
            ...
            for off_y in [-1, 0, 1]:
                ...
                for off_z in [-1, 0, 1]:
                    nle[2] = (dw[2] * off_z + dl[2]) + left_dist[2]
                    nre = nle + db
                    g, gi = self.get_box_grids_below_level(nle, nre, level, min_level)
                    mask[gi] = True
        return self.grids[mask], np.where(mask)
