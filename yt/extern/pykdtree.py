...
class KDTree(object):
    ...
    def query(self, x, k=1, eps=0, p=2, distance_upper_bound=np.inf):
        ...
        if retshape != ():
            ...
            for c in np.ndindex(retshape):
                hits = self.__query(
                    x[c], k=k, p=p, distance_upper_bound=distance_upper_bound
                )
                if k is None:
    ...
    def __query_ball_point(self, x, r, p=2.0, eps=0):
        ...
        def traverse_checking(node, rect):
            if rect.min_distance_point(x, p) >= r / (1.0 + eps):
                return []
            elif rect.max_distance_point(x, p) < r * (1.0 + eps):
                ...
                return traverse_checking(node.less, less) + traverse_checking(
        ...
        def traverse_no_checking(node):
            if isinstance(node, KDTree.leafnode):
                ...
                return traverse_no_checking(node.less) + traverse_no_checking(
                    node.greater
                )
    ...
    def query_ball_point(self, x, r, p=2.0, eps=0):
        ...
        if x.shape[-1] != self.m:
            raise ValueError(
                "Searching for a %d-dimensional point in a %d-dimensional KDTree"
                % (x.shape[-1], self.m)
            )
        if len(x.shape) == 1:
    ...
    def query_ball_tree(self, other, r, p=2.0, eps=0):
        ...
        def traverse_checking(node1, rect1, node2, rect2):
            if rect1.min_distance_rectangle(rect2, p) > r / (1.0 + eps):
                return
            elif rect1.max_distance_rectangle(rect2, p) < r * (1.0 + eps):
                ...
                if isinstance(node2, KDTree.leafnode):
                    ...
                    for i in node1.idx:
                        results[i] += node2.idx[
                            minkowski_distance(d, self.data[i], p) <= r
                        ].tolist()
                else:
        ...
        traverse_checking(
            self.tree,
            Rectangle(self.maxes, self.mins),
            other.tree,
            Rectangle(other.maxes, other.mins),
        )
        return results
    ...
    def query_pairs(self, r, p=2.0, eps=0):
        ...
        def traverse_checking(node1, rect1, node2, rect2):
            ...
            if isinstance(node1, KDTree.leafnode):
                ...
                traverse_checking(node1.less, less, node2, rect2)
                traverse_checking(node1.greater, greater, node2, rect2)
            elif rect1.min_distance_rectangle(rect2, p) > r / (1.0 + eps):
                return
            elif rect1.max_distance_rectangle(rect2, p) < r * (1.0 + eps):
                traverse_no_checking(node1.less, node2)
                traverse_no_checking(node1.greater, node2)
    ...
    def count_neighbors(self, other, r, p=2.0):
        ...
        def traverse(node1, rect1, node2, rect2, idx):
            ...
            if isinstance(node1, KDTree.leafnode):
                if isinstance(node2, KDTree.leafnode):
                    ds = minkowski_distance(
                        self.data[node1.idx][:, np.newaxis, :],
