...
def test_sanitize_valid_normal_vector():
    ...
    # because at the way normal "vectors" work isn't cleary
    ...
    for expected, user_inputs in axis_label_from_inputs.items():
        for ui in user_inputs:
            assert _sanitize_normal_vector(ds, ui) == expected
    ...
    for ui in [(1, 1, 1), [0.0, -3, 1e9], np.ones(3, dtype="int8")]:
        res = _sanitize_normal_vector(ds, ui)
...
def test_reject_invalid_normal_vector():
    ...
    for ui in [
        ...
        assert_raises(ValueError, _sanitize_normal_vector, ds, ui)
