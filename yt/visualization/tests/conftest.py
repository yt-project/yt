...
test_params = {
    ...
    "test_mesh_slices_amr": {
        "field": [
            fake_amr_ds().field_list,
            (f.__repr__() for f in fake_amr_ds().field_list),
        ]
    },
    "test_mesh_slices_tetrahedral": {
        "field": [
            fake_tetrahedral_ds().field_list,
            (f.__repr__() for f in fake_amr_ds().field_list),
        ],
        "idir": [(0, 1, 2), ("0", "1", "2")],
    },
    "test_mesh_slices_hexahedral": {
        "field": [
            fake_hexahedral_ds().field_list,
            (f.__repr__() for f in fake_amr_ds().field_list),
        ],
        "idir": [(0, 1, 2,), ("0", "1", "2")],
    },
    "test_phase_plot_attributes": {"attr_name, args": [profile_pairs, profile_ids]},
}
...
def pytest_generate_tests(metafunc):
    if metafunc.function.__name__ == "test_geo_slices_amr":
        ...
        metafunc.parametrize(
            "transform", transform_list, ids=[t.__repr__() for t in transform_list]
        )
        metafunc.parametrize(
            "field, ds",
