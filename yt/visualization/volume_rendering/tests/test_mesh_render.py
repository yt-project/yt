...
from yt.testing import (ANSWER_TEST_TAG, fake_hexahedral_ds,
                        fake_tetrahedral_ds, requires_module)
...
def tet4_render(engine, field):
    ...
    im = sc.render()
    return compare(
        ds, im, "%s_render_answers_tet4_%s_%s" % (engine, field[0], field[1])
    )
...
def hex20_render(engine, field):
    ...
    im = sc.render()
    return compare(
        ds, im, "%s_render_answers_hex20_%s_%s" % (engine, field[0], field[1])
    )
