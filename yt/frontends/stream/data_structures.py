...
from .definitions import assign_particle_data, process_data, set_particle_types
...
class StreamDataset(Dataset):
    ...
    def refine_amr(
        self, refinement_criteria, fluid_operators, max_level, callback=None
    ):
        ...
        number_of_particles = np.sum(
            [grid.NumberOfParticles for grid in self.index.grids]
        )
        ...
        if number_of_particles > 0:
            ...
            for field in self.field_list:
                ...
                if (
                    ...
                    and field[0] in self.particle_types_raw
                ):
                    pdata[field] = uconcatenate(
                        [grid[field] for grid in self.index.grids]
                    )
            pdata["number_of_particles"] = number_of_particles

        last_gc = self.index.num_grids
        cur_gc = -1
        ds = self
        bbox = np.array(
        ...
        while ds.index.max_level < max_level and last_gc != cur_gc:
            mylog.info(
                "Refining another level.  Current max level: %s", ds.index.max_level
            )
            ...
            # todo : use StreamDataset.__init__() instead of this loader function
            # this is a fast way to build a copy of the dataset
            # but a lot of data is lost on the way
            ds = load_amr_grids(grid_data, ds.domain_dimensions, bbox=bbox)

            ds.particle_types_raw = self.particle_types_raw
