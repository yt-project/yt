...
class CevartGrid(AMRGridPatch):
    ...
    def __repr__(self):
        return "CevartGrid_%04i (%s)" % (self.id, self.ActiveDimensions)

class CevartHierarchy(AMRHierarchy):

    grid = CevartGrid
    _handle = None
    
    def __init__(self,pf,data_style='cevart'):
        self.data_style = data_style
        self.field_info = CevartFieldContainer()
...
class CevartStaticOutput(StaticOutput):
    _hierarchy_class = CevartHierarchy
    _fieldinfo_class = CevartFieldContainer
    _handle = None
    
    def __init__(self, filename, data_style='cevart',
        ...
        self.refine_by = 2
        self.parameters["HydroMethod"] = 'cevart'
