...
class ChimeraMesh(SemiStructuredMesh):
    ...
    def __init__(self, *args, **kwargs):
        super(ChimeraMesh, self).__init__(*args, **kwargs)

def _find_files(filename_c): #Returns a list of all files that share a frame number with the input
...
class ChimeraUNSIndex(UnstructuredIndex):
    def __init__(self, ds, dataset_type = 'chimera'):
        ...
        super(ChimeraUNSIndex, self).__init__(ds, dataset_type)
    ...
    def _initialize_mesh(self):
        self.meshes = []
        index_filenames = _find_files(self.dataset.filename) #Retrieves list of all datafiles with the same frame number
        if any('grid_2' in file for file in index_filenames): #Detects Yin-Yang data format
            yy = True
        else:
            yy = False
        for n , file in enumerate(index_filenames):
            with h5py.File(file,"r") as f:
                ...
                l = int(file[-5:-3])-1   #Pulls the subgrid number from the data file name
                if nmz > 2:
                    k = f["fluid"]["entropy"].shape[0]
                    r = f["mesh"]["x_ef"][:-2]
                    theta = f["mesh"]["y_ef"][:]
                    phi = f["mesh"]["z_ef"][k*l:k*(l+1)+1] #Pulls only the individual subgrid's band of phi values
                ...
                if yy == True:
                    ...
                    if 'grid_2' in file: #Applies transform to meshes in the rotated grid
                        x_hold = -x
                        y_hold = z
                        z_hold = y
                    else:
                ...
                '''
                connectivity = np.zeros(((nyd-1)*(nxd-1)*(nzd-1),8), dtype="int64", order="C") #Creates scaffold array
                connectivity[0] = [0,1,nzd,(nzd+1),(nyzd),(nyzd+1),(nyzd+nzd),(nyzd+nzd+1)] #Manually defines first coordinate set
            
            
                for p in range(nzd-1): #Increments first row around phi to define an arc of cells
                    if p > 0:
                        connectivity[p] = connectivity[p-1] + 1
                for t in range(nyd-1): #Increments this arc around theta to define a shell
                    if t > 0:
                        connectivity[t*(nzd-1):(t+1)*(nzd-1)] = connectivity[(t-1)*(nzd-1):t*(nzd-1)] + nzd
                for r in range(nxd-1): #Increments this shell along r to define a sphere
                    if r > 0:
                        connectivity[r*(nyzd_):(r+1)*(nyzd_)] = connectivity[(r-1)*(nyzd_):r*(nyzd_)] + nyzd
                            
                            
                mesh = ChimeraMesh(n, self.index_filename, connectivity, coords, self) #Creates a mesh object
            
                if 'grid_' in file:
                    print(f'Mesh {n+1}/{len(index_filenames)} generated')
                    self.meshes.append(mesh) #Adds new mesh to the list of generated meshes
...
class ChimeraDataset(Dataset):
    ...
    def __init__(self, filename, dataset_type='chimera',
        ...
        super(ChimeraDataset, self).__init__(filename, dataset_type,
    ...
    def _parse_parameter_file(self):
        with h5py.File(self.parameter_filename,"r") as f:
            if "metadata" in f:
                self.unique_identifier = f["metadata"]["date_string"][()] #Uses date as unique identifier if available, otherwise uses a default value
            else:
            ...
            if any('grid_2' in file for file in index_filenames): #Checks for Yin-Yang and changes to cartesian
                self.geometry = 'cartesian'
                self.periodicity = (False, False, False)
                dre = [f["mesh"]["x_ef"][(-1)],f["mesh"]["x_ef"][(-1)],f["mesh"]["x_ef"][(-1)]]
                dle = -1*np.array(dre)
                self.domain_dimensions = np.array([2*f["mesh"]["array_dimensions"][0],2*f["mesh"]["array_dimensions"][0],2*f["mesh"]["array_dimensions"][0]])
            else:
                self.geometry = 'spherical' #Uses default spherical geometry
                self.periodicity = (False, False, True)
                dle = [f["mesh"]["x_ef"][(0)],f["mesh"]["y_ef"][(0)],f["mesh"]["z_ef"][(0)]]
                if self.domain_dimensions[2] >2:
                    dre = [f["mesh"]["x_ef"][(-3)],f["mesh"]["y_ef"][(-1)],f["mesh"]["z_ef"][(13)]]
                else:
                    dre = [f["mesh"]["x_ef"][(-1)],f["mesh"]["y_ef"][(-1)],f["mesh"]["z_ef"][(-1)]]
