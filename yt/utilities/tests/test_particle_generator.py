...
def test_particle_generator() :
    ...
    particles_per_grid1 = [grid.NumberOfParticles for grid in pf.grids]
    yield assert_equal, particles_per_grid1, particles1.NumberOfParticles
    particles_per_grid1 = [len(grid["particle_position_x"]) for grid in pf.grids]
    yield assert_equal, particles_per_grid1, particles1.NumberOfParticles

    tags = uconcatenate([grid["particle_index"] for grid in pf.grids])
    ...
    particles_per_grid2 = [grid.NumberOfParticles for grid in pf.grids]
    yield assert_equal, particles_per_grid2, particles1.NumberOfParticles+particles2.NumberOfParticles

    [grid.field_data.clear() for grid in pf.grids]
    particles_per_grid2 = [len(grid["particle_position_x"]) for grid in pf.grids]
    yield assert_equal, particles_per_grid2, particles1.NumberOfParticles+particles2.NumberOfParticles

    #Test the uniqueness of tags
    tags = np.concatenate([grid["particle_index"] for grid in pf.grids])
    tags.sort()
    ...
    old_particle_temps = [grid["particle_gas_temperature"][:particles_per_grid1[i]]
                          for i, grid in enumerate(pf.grids)]
    ...
    #Test the number of particles again
    particles_per_grid3 = [grid.NumberOfParticles for grid in pf.grids]
    yield assert_equal, particles_per_grid3, particles1.NumberOfParticles+particles2.NumberOfParticles
    particles_per_grid2 = [len(grid["particle_position_z"]) for grid in pf.grids]
    yield assert_equal, particles_per_grid3, particles1.NumberOfParticles+particles2.NumberOfParticles
