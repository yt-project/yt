...
cdef int octree_build_node(Octree * tree, long int node_idx):
    ...
    if ((tree.pend[node_idx] - tree.pstart[node_idx] > tree.n_ref) and
        ...
        if tree.num_nodes > tree.max_num_nodes - 8:
...
cdef int octree_allocate(Octree * octree, long int num_nodes):
    ...
    octree.pidx = <np.int64_t *> malloc(num_nodes * sizeof(np.int64_t))
...
cdef int octree_reallocate(Octree * octree, long int num_nodes):
    ...
    old_arr_int = octree.pidx
    octree.pidx = <np.int64_t *> malloc(num_nodes * sizeof(np.int64_t))
    for i in range(octree.num_nodes):
        octree.pidx[i] = old_arr_int[i]
    free(old_arr_int)
...
cdef class CyOctree:
    ...
    def __init__(self, np.float64_t[:, :] input_pos, left_edge = None,
                 right_edge = None, np.int64_t n_ref=32, np.uint8_t max_depth=20):

        self.n_ref = n_ref
        self.max_depth = max_depth
        self.input_positions = np.asfortranarray(input_pos)
    ...
    def _make_root(self, left_edge, right_edge):
        ...
        if left_edge is None:
            ...
            self.c_octree.pend[0] = self.c_octree.num_particles
        else:
            left_edge = left_edge.astype("float64")
            right_edge = right_edge.astype("float64")

            split = select(self.c_octree, left_edge, right_edge, <np.int64_t>0, self.c_octree.num_particles)
            self.c_octree.pstart[0] = 0
    ...
    def _allocate_octree(self):
        ...
        self.c_octree.max_num_nodes = 8**8
...
@cython.cdivision(True)
cdef int select(
        Octree * octree, np.float64_t[:] left_edge, np.float64_t[:] right_edge,
