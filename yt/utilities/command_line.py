...
def _fix_ds(arg, *args, **kwargs):
    if os.path.isdir(f"{arg}") and \
        os.path.exists(f"{arg}/{arg}"):
        ds = load(f"{arg}/{arg}", *args, **kwargs)
    elif os.path.isdir(f"{arg}.dir") and \
        os.path.exists(f"{arg}.dir/{arg}"):
...
class YTNotebookUploadCmd(YTCommand):
    ...
    def __call__(self, args):
        ...
        _id = gc.resourceLookup(
            f"/user/{username}/Public/{args.file}")["_id"]
...
class YTDeleteImageCmd(YTCommand):
    ...
    def __call__(self, args):
        headers = {'Authorization':
            f"Client-ID {ytcfg.get('yt', 'imagebin_api_key')}"}
