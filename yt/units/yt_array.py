...
def sanitize_units_add(this_object, other_object, op_string):
    ...
    if isinstance(ret, YTArray):
        ...
        if not i_units.is_dimensionless and np.any(ret):
...
def ucross(arr1, arr2, registry=None, axisa=-1, axisb=-1, axisc=-1, axis=None):
    ...
    if isinstance(units, UnitTuple):
        if not units.is_homogeneous:
            raise NotImplementedError("Taking the cross product is only "
                                      "implemented for arrays with "
                                      "homogeneous units!")
        else:
            units = units[0]
