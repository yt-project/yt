...
class YTIndexArray(YTArray):
    ...
    def _unit_repr_check_same(self, units):
        """
        Takes a UnitTuple object, or string of known unit symbol, and check that it
        is compatible with this quantity. Returns UnitTuple object.

        """
        # let UnitTuple() handle units arg if it's not already a UnitTuple obj.
        if not isinstance(units[0], Unit):
            units = UnitTuple([Unit(u, registry=self.units[0].registry) for u in units])
    ...
    def in_units(self, units):
        ...
        for i, (sunit, unit, nu) in enumerate(zip(self.units, units, new_units)):
