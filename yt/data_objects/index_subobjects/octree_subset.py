...
def cell_count_cache(func):
    def cc_cache_func(self: Any, dobj: YTRegion) -> Union[ndarray, unyt_array]:
...
class OctreeSubset(YTSelectionContainer):
    ...
    def __getitem__(self, key: Tuple[str, str]) -> NoReturn:
    ...
    def _reshape_vals(self, arr: ndarray) -> ndarray:
        nz = self.nz
        if len(arr.shape) <= 2:
    ...
    @cell_count_cache
    def select_fwidth(self, dobj: YTRegion) -> unyt_array:
