...
class TestDataContainers(unittest.TestCase):
    ...
    def test_create_firefly_object(self):
        ...
        # Test empty fields
        test_with_fields(dd, fields_to_include=[], fields_units=[])

        # Test common field (Masses)
        test_with_fields(dd, fields_to_include=["Masses"], fields_units=["code_mass"])
        ...
        with assert_raises(YTFieldNotFound) as err:
            test_with_fields(
                dd, fields_to_include=["dinos"], fields_units=["code_length"]
            )
        desired = "Could not find field dinos in ParticleData."
        assert_equal(str(err.exception), desired)

        # Test existing field tuple (gas, Temperature)
        test_with_fields(
            dd,
            fields_to_include=[("gas", "Temperature")],
            fields_units=["code_temperature"],
        )
        ...
        with assert_raises(YTFieldNotFound) as err:
            test_with_fields(
                dd,
                fields_to_include=[("PartType1", "Temperature")],
                fields_units=["code_temperature"],
            )
        desired = (
            "Could not find field ('PartType1', 'Temperature') in ParticleData.\n"
            "Did you mean:\n"
            "\t('gas', 'Temperature')"
        )
        assert_equal(str(err.exception), desired)
