...
def test_mixed_particle_mesh_profiles():
    ...
    ad = ds.all_data()
    assert_raises(
        YTIllDefinedProfile, ProfilePlot, ad, "radius", ("all", "particle_mass")
    )
    assert_raises(
        YTIllDefinedProfile,
        ProfilePlot,
        ad,
        "radius",
        [("nbody OR all", "particle_mass"), ("all", "particle_ones")],
    )
    assert_raises(
        YTIllDefinedProfile,
        ProfilePlot,
...
def test_profile_override_limits():
    ...
    profile = yt.create_profile(
        sp,
        [("gas", "density"), ("gas", "dinosaurs")],
        [("gas", "temperature")],
        override_bins={("gas", "density"): (obins, "g/cm**3"), "dinosaurs": obins},
    )
    assert_equal(ds.arr(obins, "g/cm**3"), profile.x_bins)
