...
def test_magnetic_fields():
    ...
    emag3 = (
        dd3["magnetic_field_x"] ** 2
        + dd3["magnetic_field_y"] ** 2
        + dd3[("gas", "magnetic_field_z")] ** 2
    ) / (2.0 * mu_0)
    emag3.convert_to_units("code_pressure")

    assert_almost_equal(emag1, dd1[("gas", "magnetic_energy")])
    assert_almost_equal(emag2, dd2[("gas", "magnetic_energy")])
    assert_almost_equal(emag3, dd3[("gas", "magnetic_energy")])

    assert str(emag1.units) == str(dd1[("gas", "magnetic_energy")].units)
    assert str(emag2.units) == str(dd2[("gas", "magnetic_energy")].units)
    assert str(emag3.units) == str(dd3[("gas", "magnetic_energy")].units)
