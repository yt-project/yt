...
from yt.utilities.math_utils import (get_cyl_r_component,
...
def create_relative_field(
    ...
    for d in axis_order:
        registry.add_field(
            (ftype, "relative_%s_%s" % (basename, d)),
            sampling_type="local",
...
def create_vector_fields(registry, basename, field_units, ftype="gas", slice_info=None):
    ...
    if not is_curvilinear(registry.ds.geometry):
        ...
        registry.add_field(
            (ftype, "%s_spherical_radius" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "radial_%s_absolute" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_spherical_theta" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_spherical_phi" % basename),
            sampling_type="local",
        ...
        for ax in "xyz":
            registry.add_field(
                (ftype, "cutting_plane_%s_%s" % (basename, ax)),
                sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_divergence" % basename),
            sampling_type="local",
            function=_divergence,
            units=div_units,
            validators=[ValidateSpatial(1), ValidateParameter("bulk_%s" % basename)],
        )

        registry.add_field(
            (ftype, "%s_divergence_absolute" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "tangential_over_%s_magnitude" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_cylindrical_radius" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "cylindrical_radial_%s" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "cylindrical_radial_%s_absolute" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_cylindrical_theta" % basename),
            sampling_type="local",
        ...
        registry.add_field(
            (ftype, "%s_cylindrical_z" % basename),
            sampling_type="local",
        ...
        def _cartesian_x(field, data):
            if registry.ds.geometry == "polar":
                ...
                if data.ds.dimensionality == 2:
                    ...
                    return data["%s_r" % basename] * np.cos(data["theta"]) - data[
                        "%s_theta" % basename
                    ] * np.sin(data["theta"])
                ...
                if data.ds.dimensionality == 2:
                    return data["%s_r" % basename] * np.sin(data["theta"]) + data[
                        "%s_theta" % basename
                    ] * np.cos(data["theta"])
                elif data.ds.dimensionality == 3:
                    return (
