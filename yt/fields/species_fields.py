...
def _create_fraction_func(ftype, species):
    def _frac(field, data):
        return data[ftype, f"{species}_density"] \
             / data[ftype, "density"]
    return _frac
...
def _mass_from_particle_mass_and_fraction(ftype, species):
    def _mass(field, data):
        return data[ftype, f"{species}_fraction"] \
...
def add_species_field_by_density(registry, ftype, species):
    ...
    registry.add_field((ftype, f"{species}_fraction"),
                       sampling_type="local",
    ...
    registry.add_field((ftype, f"{species}_mass"),
                       sampling_type="local",
                       function = _create_mass_func(ftype, species),
                       units = unit_system["mass"])

    registry.add_field((ftype, f"{species}_number_density"),
                       sampling_type="local",
                       function = _create_number_density_func(ftype, species),
                       units = unit_system["number_density"])

    return [(ftype, f"{species}_number_density"),
...
def add_species_field_by_fraction(registry, ftype, species):
    ...
    registry.add_field((ftype, f"{species}_density"),
                       sampling_type="local",
...
def add_species_aliases(registry, ftype, alias_species, species):
    ...
    """
    registry.alias((ftype, f"{alias_species}_density"), 
                   (ftype, f"{species}_density"))
    registry.alias((ftype, f"{alias_species}_fraction"), 
                   (ftype, f"{species}_fraction"))
    registry.alias((ftype, f"{alias_species}_number_density"), 
                   (ftype, f"{species}_number_density"))
    registry.alias((ftype, f"{alias_species}_mass"), 
                   (ftype, f"{species}_mass"))
...
def add_deprecated_species_alias(registry, ftype, alias_species, species,
    ...
    def _dep_field(field, data):
        if not isinstance(data, FieldDetector):
            issue_deprecation_warning(
                (f"The \"%s_%s\" field is deprecated. Please use \"%s_%s\" instead.") %
                (alias_species, suffix, species, suffix))
        return data[ftype, f"{species}_{suffix}"]

    registry.add_field((ftype, f"{alias_species}_{suffix}"),
                       sampling_type="local",
                       function=_dep_field,
                       units=my_units)
...
def add_nuclei_density_fields(registry, ftype):
    ...
    for element in elements:
        registry.add_field((ftype, f"{element}_nuclei_density"),
                           sampling_type="local",
