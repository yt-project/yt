...
module NN_module
    ...
    type typeNN
        ! .true./.false. for each particle if it's inside the 'real' region
        logical, pointer :: is_inside(:) => null()
        ! position of each particle in the data
        ! I don't think I need this after all
        integer, pointer :: order_index(:) => null()
        ! chainID of each particle, which can change with each step
        integer, pointer :: chainID(:) => null()
        ! ID of the densest nearest neighbor
        integer, pointer :: densestNN(:) => null()
        ! density of each particle
        real, pointer :: density(:) => null()
        ! a list of nearest neighbors IDs
        ! The list will have dimensions (nparts, num_neighbors)
        integer, pointer :: NNlist_ID(:,:) => null()
        ! a list of nearest neighbor distances, similar to above
        real, pointer :: NNlist_d(:,:) => null()
    end type typeNN
    
contains
    
    function init_NN() result (nn)
        ! instantiate the Nearest Neighbors object, NN, of type typeNN
        ...
        allocate(nn%is_inside(nparts))
        insideloop: do i=1, nparts
            ! if the particle is outside, mark it false, otherwise true
            max = minval(max_bounds(:) - pos(:,i))
            if (max < 0.) then
                nn%is_inside(i) = .false.
                cycle insideloop
            endif
            min = minval(pos(:,i) - min_bounds(:))
            if (min < 0.) then
                nn%is_inside(i) = .false.
                cycle insideloop
            endif
        ...
        ! this too - delete this?
        allocate(nn%order_index(nparts))
        do i=1,nparts
            nn%order_index(i) = i
        end do
        ...
        allocate(nn%NNlist_d(nparts, num_neighbors))
        
        return
    end function init_NN
...
contains
    ...
    subroutine smDensitySym(index,nn)
        ...
        ih2 = 4.0/nn%NNlist_d(index,num_neighbors)
        fNorm = 0.5 * sqrt(ih2) * ih2 / 3.1415926535897931
        do i=1,num_neighbors
            pj = nn%NNlist_ID(index,i)
            r2 = nn%NNlist_d(index,i) * ih2
            rs = 2.0 - sqrt(r2)
            if (r2 < 1.0) then
                rs = (1.0 - 0.75*rs*r2)
            else
            ...
            rs = rs * fNorm
            nn%density(index) = nn%density(index) + rs * mass(pj)
            nn%density(pj) = nn%density(pj) + rs * mass(index)
        end do
        
        return
    end subroutine smDensitySym

    subroutine densestNN(index,nn)
        ! mark the densest nearest neighbor for this particle
        integer, intent (In) :: index
        type(typeNN), pointer :: nn
        integer :: j, nn_temp, nn_index
        real :: nndens
        
        nndens = 0.0
        do j=1,num_neighbors
            nn_temp = nn%NNlist_ID(index,j)
            if (nn%density(nn_temp) > nndens) then
                nn_index = nn_temp
                nndens = nn%density(nn_temp)
            endif
        end do
        nn%densestNN(index) = nn_index
        return
